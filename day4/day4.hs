...
type BoardBland = (Int, Array Pos Cell) -- Board without identifier
...
checkRows b = or $ map (flip (checkBingo b) (1, 0)) (getPosLine (1, 1) (0, 1))
checkCols b = or $ map (flip (checkBingo b) (0, 1)) (getPosLine (1, 1) (1, 0))
...
parseInp (x:xs) = (parseInts x, parseBoards xs) where
    parseBoard :: [String] -> BoardBland
    parseBoard = (,) (-1) . listArray ((1,1),(5,5)) . concat . map parseInts

    -- Associate each board with an ID
    identify :: [BoardBland] -> [Board]
